...
sample.x <- as.data.table(matrix(0, ncol = 0, nrow = nrow(df)/10)) 
```
...
for(i in 1:loop) {
  ...
  sample.x <- sample.x[x>-100 & x<100]
  sample <- dt[sample.x, on="x"]
  ...
  results[i,c(3,4)]<-results.current$bws[bc+1,1:2]
