...
for(i in 1:loop) {
  ...
  results[i,3 := .(results.current$bws[bc+1,][[1]])]
  results[i,4 := .(results.current$bws[bc+1,][[2]])]
  results[i,2 := sum(obw_l,obw_r)]
  
  if (exercise=="zero") {
    sample.exercise = copy(sample)
    sample.exercise[, y := fifelse(x %between% c(-results[i,3], results[i,4]), y,0)]
  ...
  results.exercise[i,3 := .(results.current$bws[bc+1,][[1]])]
  results.exercise[i,4 := .(results.current$bws[bc+1,][[2]])]
